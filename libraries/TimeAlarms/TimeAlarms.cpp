...
AlarmID_t TimeAlarmsClass::triggerOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){   // trigger once at the given time_t
	if( value > 0)
		return create( value, onTickHandler, IS_ONESHOT, dtExplicitAlarm, true, eventName, tag1, tag2 );
	else
...
AlarmID_t TimeAlarmsClass::alarmOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){   // trigger once at the given time of day
	if( value <= SECS_PER_DAY)
		return create( value, onTickHandler, IS_ONESHOT, dtDailyAlarm, true, eventName, tag1, tag2 );
	else
		return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}

AlarmID_t TimeAlarmsClass::alarmOnce(const int H,  const int M,  const int S,OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){   // as above with HMS arguments
	return create( AlarmHMS(H,M,S), onTickHandler, IS_ONESHOT, dtDailyAlarm, true, eventName, tag1, tag2 );
}

AlarmID_t TimeAlarmsClass::alarmOnce(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){  // as above, with day of week 
	return create( (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S), onTickHandler, IS_ONESHOT, dtWeeklyAlarm, true, eventName, tag1, tag2 );   
}

// this method will now return an error if the value is greater than one day - use DOW methods for weekly alarms   
AlarmID_t TimeAlarmsClass::alarmRepeat(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){ // trigger daily at the given time
	if( value <= SECS_PER_DAY)
		return create( value, onTickHandler, IS_REPEAT, dtDailyAlarm, true, eventName, tag1, tag2 );
	else
		return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}

AlarmID_t TimeAlarmsClass::alarmRepeat(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){ // as above with HMS arguments
	return create( AlarmHMS(H,M,S), onTickHandler, IS_REPEAT, dtDailyAlarm, true, eventName, tag1, tag2 );
}

AlarmID_t TimeAlarmsClass::alarmRepeat(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){  // as above, with day of week 
	return create( (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S), onTickHandler, IS_REPEAT, dtWeeklyAlarm, true, eventName, tag1, tag2 );      
}

AlarmID_t TimeAlarmsClass::timerOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){   // trigger once after the given number of seconds 
	return create( value, onTickHandler, IS_ONESHOT, dtTimer, true, eventName, tag1, tag2 );
}

AlarmID_t TimeAlarmsClass::timerOnce(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){   // As above with HMS arguments
	return create( AlarmHMS(H,M,S), onTickHandler, IS_ONESHOT, dtTimer, true, eventName, tag1, tag2 );
}

AlarmID_t TimeAlarmsClass::timerRepeat(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){ // trigger after the given number of seconds continuously
	return create( value, onTickHandler, IS_REPEAT, dtTimer, true, eventName, tag1, tag2 );
}

AlarmID_t TimeAlarmsClass::timerRepeat(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2){ // trigger after the given number of seconds continuously
	return create( AlarmHMS(H,M,S), onTickHandler, IS_REPEAT, dtTimer, true, eventName, tag1, tag2 );
}
	...
	if(! isServicing)
	{
		...
		for( servicedAlarmId = 0; servicedAlarmId < dtNBR_ALARMS; servicedAlarmId++)
		{
			if( Alarm[servicedAlarmId].Mode.isEnabled && (now() >= Alarm[servicedAlarmId].nextTrigger)  )
			{
				...
				if( TickHandler != NULL) {        
					(*TickHandler)(Alarm[servicedAlarmId].tag1, Alarm[servicedAlarmId].tag2);     // call the handler  
				}
			}
		}
		isServicing = false;
...
// returns the AlarmClass of the next scheduled alarm, or NULL if none
AlarmClass* TimeAlarmsClass::getNextTriggerAlarm()
{
	time_t nextTrigger = 0xffffffff;  // the max time value
	int next_id = -1;
	...
	for(uint8_t id = 0; id < dtNBR_ALARMS; id++)
	{
		if(isAllocated(id) && Alarm[id].eventName)
		{
			if(Alarm[id].nextTrigger <  nextTrigger)
			{
				nextTrigger = Alarm[id].nextTrigger;	
	...
	if  (next_id < 0)
		return NULL;
	return  &Alarm[next_id]; 
}

// returns the AlarmClass of the alarm, or NULL if none
AlarmClass* TimeAlarmsClass::getAlarm(AlarmID_t id)
{
	if (isAlarm(id) && Alarm[id].eventName)
		return  &Alarm[id]; 
	else 
		return NULL;
}
...
AlarmID_t TimeAlarmsClass::create( time_t value, OnTick_t onTickHandler, uint8_t isOneShot, dtAlarmPeriod_t alarmType, uint8_t isEnabled, const char* eventName, byte tag1, int tag2) 
{
