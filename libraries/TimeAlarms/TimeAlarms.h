...
typedef boolean (*OnTick_t)(byte tag1, int tag2);  // alarm callback function typedef 
...
private:
	...
	AlarmID_t create( time_t value, OnTick_t onTickHandler, uint8_t isOneShot, dtAlarmPeriod_t alarmType, uint8_t isEnabled, const char* eventName, byte tag1, int tag2);

public:
	TimeAlarmsClass();
	// functions to create alarms and timers

	AlarmID_t triggerOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);   // trigger once at the given time_t

	AlarmID_t alarmRepeat(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);                    // trigger daily at given time of day
	AlarmID_t alarmRepeat(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2); // as above, with hms arguments
	AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2); // as above, with day of week 

	AlarmID_t alarmOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);                     // trigger once at given time of day
	AlarmID_t alarmOnce( const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);  // as above, with hms arguments
	AlarmID_t alarmOnce(const timeDayOfWeek_t DOW, const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2); // as above, with day of week 

	AlarmID_t timerOnce(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);   // trigger once after the given number of seconds 
	AlarmID_t timerOnce(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);   // As above with HMS arguments

	AlarmID_t timerRepeat(time_t value, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2); // trigger after the given number of seconds continuously
	AlarmID_t timerRepeat(const int H,  const int M,  const int S, OnTick_t onTickHandler, const char* eventName, byte tag1, int tag2);   // As above with HMS arguments
